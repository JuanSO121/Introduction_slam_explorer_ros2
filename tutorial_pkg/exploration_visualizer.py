...
class ExplorationVisualizer(Node):
    ...
    def create_goal_history_markers(self):
        ...
        for i, goal in enumerate(self.exploration_goals_history):
            ...
            marker.pose.position.x = goal['position'][0]
            marker.pose.position.y = goal['position'][1]
    ...
    def calculate_exploration_stats(self):
        ...
        stats = {
            ...
            'goals_sent': len(self.exploration_goals_history),
            'map_growth_rate': 0,
            'current_map_size': 0,
            'time_since_map_update': current_time - self.last_map_update
        }
        ...
        if len(self.map_size_history) >= 2:
            ...
            stats['current_map_size'] = recent_map['size']
            time_diff = recent_map['time'] - old_map['time']
            
            if time_diff > 0:
                size_diff = recent_map['size'] - old_map['size']
                stats['map_growth_rate'] = size_diff / time_diff  # células/segundo
    ...
    def update_visualization(self):
        ...
        status_markers.markers.append(
            self.create_status_text_marker(
                5, status_text, [-2, 0.5, 0.5], status_color
            )
        )
