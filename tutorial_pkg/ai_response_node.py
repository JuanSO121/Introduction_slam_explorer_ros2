...
class AIResponseNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('status_publish_rate', 2.0)   # cada 2 segundos
        self.declare_parameter('heartbeat_interval', 30.0)   # latido cada 30 segundos

        # Obtener parámetros con fallback seguro
        try:
            self.status_publish_rate = self.get_parameter(
                'status_publish_rate').get_parameter_value().double_value
            self.heartbeat_interval = self.get_parameter(
                'heartbeat_interval').get_parameter_value().double_value
        except Exception as e:
            self.get_logger().warn(f"⚠️ Error al leer parámetros: {e} - usando valores por defecto")
            self.status_publish_rate = 2.0
        ...
        self.exploration_control_subscription = self.create_subscription(
            String, '/exploration_control', self.exploration_control_callback, 10)

        # Publicadores
        self.ai_status_publisher = self.create_publisher(String, '/ai_status', 10)
        ...
        # Timers (usando parámetros configurables)
        self.create_timer(self.status_publish_rate, self.publish_robot_state)
        self.create_timer(1.0, self.check_robot_activity)  # se mantiene fijo

        self.get_logger().info('🧠 AI Response Node (tutorial_pkg) iniciado')
        self.get_logger().info(f'📡 Publicando estado cada {self.status_publish_rate:.1f}s')
        self.get_logger().info(f'❤️ Heartbeat configurado a {self.heartbeat_interval:.1f}s')
...
def main(args=None):
    ...
    try:
        ai_response_node.get_logger().info('🧠 AI Response Node ejecutándose...')
